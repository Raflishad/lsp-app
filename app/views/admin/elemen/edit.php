      ...
      <div class="card">
        <div class="card-content">
          <div class="card-body card-dashboard table-responsive">
            <h2 class="card-title">Edit Elemen</h2>
            <form method="POST">
              <div class="row justify-content-start mt-2">
                <div class="col-md-6">
                  <div class="form-body">
                    ...
                    <input type="hidden" name="id" value="<?= isset($data['ID_ELEMEN']) ?>">
                    <label>Unit</label>
                    <select name="unitId" class="form-control" required>
                        <option value="">-- Pilih Unit --</option>
                        <?php foreach ($unit as $u): ?>
                            <option value="<?= $u['ID_UNIT'] ?>"
                                <?= ($u['ID_UNIT'] == $data['ID_UNIT']) ? 'selected' : '' ?>>
                                <?= htmlspecialchars($u['KODE_UNIT']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <label class="mt-2">Nomor</label>
                    <input type="text" class="form-control" name="nomor" value="<?= htmlspecialchars($data['NOMOR_ELEMEN']) ?>" required>
                    <label class="mt-2">Elemen</label>
                    <input type="text" class="form-control" name="elemen" value="<?= htmlspecialchars($data['ELEMEN']) ?>" required>
                    <div class="mt-3">
